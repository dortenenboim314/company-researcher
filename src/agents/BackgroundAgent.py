...
from api_clients.tavily_client import TavilyBatchSearchInput, TavilyClient
...
class BackgroundAgent:
    ...
    async def run(self, state: ResearchState) -> ResearchState:
        ...
        site_content = self.tavily_client.crawl(state['company_url'])
        ...
        search_input = self._generate_queries_for_missing_info(grounded_info)
    ...
    def process_background_data(self, grounded_info: str, search_output: Dict[str, Any]) -> Any:
        ...
        Your given a grounded information about a company, and search results for missging information.
    ...
    def _summarize_to_grounded_info(self, site_content: List[tuple[str, str]]) -> str:
    ...
    def _generate_queries_for_missing_info(self, grounded_info: str, max_queries) -> TavilyBatchSearchInput:
        ...
        Do not generate queries that are speficific to finance, news or market data. You should only generate queries that are relevant to the company's background, such as its history, founding, main business activities, key milestones, and current status.
        ...
        response = self.llm.with_structured_output(TavilyBatchSearchInput).ainvoke([HumanMessage(content=prompt)])
        return response
